...
export default function PitchDeck({ idea, auditResult }: PitchDeckProps) {
  ...
  const generateDeck = async () => {
    ...
    try {
      ...
      const res = await actions.createPitchDeck(idea, auditResult, token);
  ...
  if (!deck && !loading) {
    return (
      <div className="max-w-3xl mx-auto text-center space-y-6 animate-fade-in p-12 bg-black/40 border border-white/10 rounded-3xl">
        <h2 className="text-3xl font-black text-white uppercase tracking-tighter">Pitch Deck Generator</h2>
        <p className="text-gray-400 text-sm max-w-xl mx-auto">
          Convert your raw idea and the audit's Brutal Truths into a tailored 10-slide pitch deck structure designed to actually raise capital.
        </p>
        <button
  ...
  if (loading) {
    return (
      <div className="max-w-3xl mx-auto text-center space-y-6 py-20 animate-fade-in">
        ...
        <p className="text-white font-bold animate-pulse text-sm">Architecting slides...</p>
      </div>
    );
  }
      ...
      <div className="bg-black border border-white/20 rounded-2xl p-12 aspect-video flex flex-col justify-center relative shadow-2xl overflow-hidden group">
        ...
        <div className="relative z-10 space-y-8">
          <div>
            ...
            <h2 className="text-4xl sm:text-5xl font-black text-white tracking-tighter leading-tight max-w-2xl">
              {slide.subtitle}
            </h2>
          </div>
          ...
          <ul className="space-y-4 max-w-xl">
            {slide.bulletPoints.map((bp, i) => (
              <li key={i} className="flex gap-4 text-gray-300 text-lg sm:text-xl">
