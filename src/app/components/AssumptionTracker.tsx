...
import { IdeaInput } from './SprintPlan'; // Reusing the type
...
export default function AssumptionTracker({ idea, auditResult }: AssumptionTrackerProps) {
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        // Ensure we still have the ones from the audit if they changed, but for simplicity just use saved if it exists
  ...
  const handleAddEvidence = (id: string) => {
    ...
    setAssumptions(prev => prev.map(a => {
      if (a.id === id) {
        return {
          ...a,
          evidence: [...a.evidence, { text, date: new Date().toLocaleDateString() }],
  ...
  const triggerRescore = async () => {
    ...
    try {
      ...
      const evidenceLog = assumptions.map(a => ({
        assumption: a.text,
        status: a.status,
        evidence: a.evidence.map(e => e.text)
      }));
    ...
    <div className="max-w-5xl mx-auto space-y-12 animate-fade-in pb-20">
      <div className="text-center space-y-4">
        ...
        <p className="text-gray-400 text-sm max-w-xl mx-auto">
          Ideas aren't validated in Notion docs. They are validated in the market. Log your field evidence against the critical assumption stack here.
        </p>
      </div>
        ...
        {assumptions.map(a => (
          <div key={a.id} className="bg-black/40 border border-white/10 rounded-2xl flex flex-col overflow-hidden">
                ...
                <div className="space-y-3 mt-4 pt-4 border-t border-white/5">
                  ...
                  {a.evidence.map((ev, i) => (
                    <div key={i} className="bg-white/5 p-3 rounded-xl border border-white/5 text-sm text-gray-300">
                      <span className="text-[10px] text-purple-400 block mb-1">{ev.date}</span>
                      {ev.text}
                    </div>
                  ))}
                </div>
            ...
            <div className="p-4 bg-white/5 border-t border-white/10 flex gap-2">
              <input 
                type="text" 
                placeholder="Log field evidence (e.g. '3 users pre-paid $50')..." 
